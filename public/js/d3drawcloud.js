    ...
    for (var i = 0; i < wordarray.length; i++) {
        if (wordarray[i].length > 3 && typeof wordarray[i] !== "undefined") { // ignore words less than 4 characters
            if (!wordExists(freqlist, wordarray[i])) { // add new words to the freq list
                ...
                //   if (freqlist[i].text == wordarray[i]) {
                //     freqlist[i].size += 1;
                //   }
                // }
            }
...
function wordExists(freqlist, word) {
    var exists = false;
    for (var i = 0; i < freqlist.length; i++) {
        if (freqlist[i].text == word) {
            exists = true;
        }
    }
    return exists;
...
function displayWordCloud(frequency_list) {
    ...
    d3.layout.cloud().size([800, 300]).words(frequency_list).rotate(0).fontSize(function (d) {
        return d.size;
    }).on("end", draw).start();

    function draw(words) {
        d3.select("#cloud_div").append("svg").attr("width", 850).attr("height", 350).attr("class", "wordcloud").append("g")
